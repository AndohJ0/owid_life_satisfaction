...
## Import data
cantril <- 
  read_csv(here("share-of-people-who-say-they-are-happy.csv")) %>% 
...
cantril %>% 
  ...
  mutate(`Absolute change` = round(`2022` - `1993`, 2)) %>% 
...
```{r}
cantril %>% 
  mutate(`Happy score` = round(`Happiness: Happy (aggregate)`, 2)) %>%
  plot_geo(width = 940) %>% 
  add_trace(
    z = ~`Happy score`,
    color = ~`Happy score`,
...
happy_chart <- 
  cantril %>% 
  filter(Country %in% c("Switzerland", "Australia", "Germany", "Bolivia", "Colombia", "United States")) %>%
...
[https://ourworldindata.org/grapher/share-of-people-who-say-they-are-happy](https://ourworldindata.org/grapher/share-of-people-who-say-they-are-happy) | CC BY
